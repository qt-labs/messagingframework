...
QMailMessage EmailComposerInterface::message() const
{
    ...
    if (!fromAddress.isEmpty()) {
        // Find the account matching the from setting
        key &= QMailAccountKey(QMailAccountKey::FromAddress, fromAddress);
    }

    QMailAccountKey transmitKey(QMailAccountKey::Status, QMailAccount::CanTransmit, QMailDataComparator::Includes);
    QMailAccountIdList ids(QMailStore::instance()->queryAccounts(key & transmitKey));
    if (!ids.isEmpty()) {
        QMailAccount account(ids.first());
    ...
    if (mail.multipartType() == QMailMessagePartContainer::MultipartNone) {
        ...
        for ( uint i = 0; i < mail.partCount(); ++i ) {
            ...
            if (textPart == -1 && part.contentType().type().toLower() == "text") {
                ...
                QString fileName(part.writeBodyTo(Qtopia::tempDir()));
                if (fileName.isEmpty()) {
